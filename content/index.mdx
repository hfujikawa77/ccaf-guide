...
CCA-F は「本番グレードの Claude システムをアーキテクチャレベルで設計・実装できるか」を検証する試験です。知識の暗記ではなく **実際の本番環境での判断力** が問われます。
...
| 2 | 感情スコアでエスカレーション | 感情とケース複雑度は無関係。**明示的な基準 + few-shot** |
| 3 | Batch API をブロッキング処理に使用 | pre-merge チェック等 SLA 必要なものは **同期 API 必須** |
| 4 | コンテキストウィンドウを拡大して attention dilution を解決 | サイズ ≠ 注意品質。**per-file パスへ分割** |
| 5 | サブエージェント timeout を empty success として返す | エラー情報なしでコーディネーターが誤判断。**構造化エラー** |
| 6 | 自己レビュー（同じセッション） | 認知バイアスあり。**独立したインスタンス** |
| 7 | 競合ソースの一方を選択 | 出所情報が消失。**両方を帰属付きで注釈** |
